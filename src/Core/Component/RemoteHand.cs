...
public class RemoteHand : MonoBehaviour {
	...
	[Header("拖拽设置（被敌对物体拉拽）")]
	[Tooltip("基础拖拽力度")]
	[SerializeField] public float basePullStrength = 0.2f;

	[Tooltip("拖拽吸附点偏移")]
	[SerializeField] public Vector3 pullAttachOffset = Vector3.up * -0.5f;

	[Tooltip("拖拽最大有效距离")]
	[SerializeField] public float maxPullDistance = 10f;

	[Tooltip("拖拽超时时间（秒）")]
	private const float PULL_TIMEOUT_SECONDS = 1.0f;

	// 玩家ID,用于识别玩家
	public IDType playerId;    
	// 状态标志
	private bool _isTeleporting;           // 是否正在传送
	private float _lastPullTimestamp;      // 上次拖拽时间戳
	private bool _isBeingPulled;           // 是否正在被拖拽
	private bool _wasBeingPulled;          // 上一帧是否被拖拽 (用于检测状态变化)
	private bool _wasBeingGrabbed;         // 是否被抓取 (由外部设置, 可能影响位置同步逻辑)
	...
	private void FixedUpdate() {
		// 拖拽超时检测
		if (_isBeingPulled && Time.time - _lastPullTimestamp > PULL_TIMEOUT_SECONDS) {
			_isBeingPulled = false;
			MPEventBusGame.NotifyRemotePull(playerId, _isBeingPulled);
		}
	...
	private void ApplyPullPhysics() {
		...
		float distance = Vector3.Distance(
			transform.position + pullAttachOffset, 
		...
		// 平滑力度
		_currentPullStrength = Mathf.Lerp(
			_currentPullStrength,
			basePullStrength,
		...
		Vector3 finalVelocity = pullDirection * Time.fixedDeltaTime * _currentPullStrength * distanceFactor;
	...
	/// <param name="targetPlayerId">该玩家ID是否被抓取/被拖拽</param>
	public void UpdateFromHandData(ref PlayerData.HandData handData, IDType targetPlayerId = 0) {
		...
		if (handData.IsGrabbing(targetPlayerId)) {
			_targetWorldPosition = handData.DesiredPosition;
			_isBeingPulled = true;
			_lastPullTimestamp = Time.time;
		} else {
			_isBeingPulled = false;
		}

		// 被抓取状态变化通知
		if (handData.IsGrabbing(targetPlayerId) != _wasBeingGrabbed) {
			MPEventBusGame.NotifyRemoteGrab(playerId, !_wasBeingGrabbed);
			_wasBeingGrabbed = !_wasBeingGrabbed;
		}

		// 被拖拽状态变化通知
		if (_isBeingPulled != _wasBeingPulled) {
			MPEventBusGame.NotifyRemotePull(playerId, _isBeingPulled);
